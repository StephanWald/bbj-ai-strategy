...
must_haves:
  truths:
    - "sources.toml defines all 9 source entries (6 parsers, 3 MDX dirs) with paths, parser types, generation tags, and enabled flags"
...
<objective>
Create the source configuration infrastructure for all 6 BBj documentation sources (9 entries with 3 MDX directories), update the MdxParser for multi-directory support with unique source_url prefixes, and fix the report module's source_url pattern matching.
...
<task type="auto">
  ...
  <action>
    ...
    **Top-level key:**
    - `data_dir = "/Users/beff/_workspace"` -- base directory for relative path resolution. Override with `DATA_DIR` env var.
...
<success_criteria>
1. sources.toml exists with 9 source entries (6 parser types, 3 MDX directories)
2. source_config.py loads, validates, and resolves paths from sources.toml
3. MdxParser produces unique source_url prefixes per directory (mdx-dwc://, mdx-beginner://, mdx-db-modernization://)
4. report.py SQL patterns match all actual source_url prefixes
5. All existing tests pass (no regressions from MdxParser or report changes)
</success_criteria>
