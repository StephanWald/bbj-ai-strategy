...
must_haves:
  truths:
    ...
    - "Each source runs through the existing run_pipeline() with correct parser, embedder, and connection"
...
<task type="auto">
  ...
  <action>
    ...
    **Orchestration Flow:**
    ...
    6. **Run ingestion loop:**
       ...
       g. Between sources, run `conn.execute("SELECT 1")` as keepalive to prevent idle timeout.
    ...
    **Parser Factory** (`_create_parser_for_source`):
    Create a private function that maps source config to parser instances:
    ...
    **Important implementation details:**
    - Open a FRESH database connection per source (not one shared connection) to avoid timeout issues during long runs. Use `get_connection_from_settings(settings)` per source, close after each.
  ...
  <done>
    ingest_all.py exists with Click CLI, parser factory for all 6 parser types, fail-fast validation (paths + DB + embedder), clean/resume/parallel/verbose/source flags, per-source pipeline execution, continue-on-failure, resume state tracking via JSON, and formatted summary table output.
  </done>
</task>
...
5. **Existing tests pass:** `cd rag-ingestion && python -m pytest tests/ -v 2>&1 | tail -10`

6. **Ruff lint:** `cd rag-ingestion && python -m ruff check src/bbj_rag/ingest_all.py src/bbj_rag/source_config.py`
</verification>
...
3. Parser factory correctly instantiates all 6 parser types from source config entries
