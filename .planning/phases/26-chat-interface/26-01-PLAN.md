  ...
  <action>
    ...
    **chat/stream.py — `stream_chat_response(messages, search_results, settings, low_confidence)`:**
    ...
    - Is an async generator that yields dicts in sse-starlette format:
      1. First yield: `{"event": "sources", "data": json.dumps(sources_list)}` where sources_list is an array preserving the same order as the system prompt numbering (index 0 = Source 1, etc.), each entry containing `{"index": i, "title", "url", "source_type", "low_confidence"}`. Claude receives the full display_url per source in the system prompt, so it outputs complete markdown links like `[Source 1](https://documentation.basis.cloud/...)` — no frontend URL mapping needed.
