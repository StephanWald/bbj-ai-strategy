...
<task type="auto">
  ...
  <action>
    ...
    **`displaySources(sources, containerEl)` function:**
    - Creates a complementary "Sources" reference section below the answer, like Wikipedia's References section at the bottom of an article. This does NOT replace Claude's inline citations — Claude weaves source links directly into the answer text (e.g., "according to the [Flare Docs](url)..."). The displaySources section provides a consolidated list of all sources referenced, so engineers can see at a glance what backed the answer.
    - Create `.sources-list` div with a "Sources" label.
