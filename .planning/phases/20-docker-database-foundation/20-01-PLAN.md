...
- `shm_size: 256mb` (documented minimum for HNSW index builds per STATE.md blockers; add a YAML comment noting this is tunable upward for larger corpora)
...
- `env_file: - path: .env, required: true` (fails fast if .env is missing, consistent with the fail-fast principle for configuration)
...
**.env.example** -- Template with all required vars, documented with comments. **Note:** All app-level environment variables use the `BBJ_RAG_` prefix because the existing pydantic-settings configuration in `config.py` sets `env_prefix="BBJ_RAG_"`. The Settings field names are `db_host`, `db_port`, etc., but the corresponding env var names are `BBJ_RAG_DB_HOST`, `BBJ_RAG_DB_PORT`, etc.
...
**IMPORTANT:** For the database check, the health endpoint needs the separate DB fields that Plan 02 will add to Settings. For now, use `settings.database_url` (the existing field) for the connection. In Plan 02, `database_url` becomes a computed property (not a Field), so code using `settings.database_url` will continue to work without changes. Plan 02 will optionally update health.py to use keyword args (preferred) but the existing `database_url` property approach also works.
...
**Note:** HTTP status code behavior (503 on degraded) will be validated during 20-02's Docker integration test where the actual containers are running and real DB/Ollama connectivity can be tested.
  </verify>
  <done>
