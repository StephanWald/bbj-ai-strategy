...
**health.py** -- OPTIONALLY update to use the refactored Settings with separate DB fields:

Since `database_url` is now a computed property on Settings (from Task 1's config.py refactor), the existing health.py code from Plan 20-01 that uses `settings.database_url` will continue to work without changes. However, the PREFERRED approach is to update the health endpoint to connect using keyword args from Settings: use `psycopg.connect(host=..., port=..., user=..., password=..., dbname=...)` directly in the health check, or call `get_connection_from_settings`. Using keyword args is preferred for clarity, but either approach is acceptable.
