# Requirements Archive: v1.5 Alpha-Ready RAG System
...
- [x] **PERF-01**: Concurrent embedding workers process multiple chunks in parallel via asyncio
- [x] **PERF-02**: Persistent HTTP connection reuse for Ollama embedding calls across batches
- [x] **PERF-03**: `bbj-ingest-all --parallel` flag enables concurrent mode with configurable worker count
  - *Note: Changed to parallel-by-default with `--sequential` fallback*
...
- **COMP-F01**: Validation error feedback loop (send bbjcpl errors back to LLM for self-correction)
  - *Note: Implemented in v1.5 as part of COMP-02 (auto-fix loop with 3 attempts)*
...
**Shipped:** 18 of 18 v1.5 requirements

**Adjusted:**
- PERF-03: Changed from `--parallel` flag to parallel-by-default with `--sequential` fallback (user-friendlier default)
- COMP-F01 (validation feedback loop): Originally deferred, but implemented as part of COMP-02 during Phase 28
