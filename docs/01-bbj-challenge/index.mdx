...
The BBj AI strategy is in active development. Here is where things stand as of early 2026:

- **Problem validated:** Testing with ChatGPT, Claude, and GitHub Copilot confirms that all major LLMs produce incorrect BBj code. The hallucination pattern is consistent -- they default to VB/VBA-style syntax.
- **Training data structure defined:** A generation-tagged format has been designed that labels examples as universal (`"all"`), generation-specific (`"character"`, `"vpro5"`, `"bbj-gui"`, `"dwc"`), or multi-generation subsets. See [Fine-Tuning the Model](/docs/fine-tuning) for details.
- **Base model candidates identified:** The Qwen2.5-Coder family (7B/14B/32B parameters) is the leading candidate for fine-tuning, offering strong code comprehension at sizes practical for self-hosting.
- **Infrastructure approach selected:** Ollama for model serving, enabling both internal deployment and customer self-hosting with full data privacy.
...
The chapters that follow detail the architecture, fine-tuning approach, IDE integration, and implementation roadmap for delivering AI-powered BBj development tools.
