...
The highest-priority risk (medium likelihood, high impact) is model hallucination. This is mitigated by a two-tier validation strategy described in [Chapter 4](/docs/ide-integration): first, the Langium parser catches structural syntax errors (the same parser that powers the language server's diagnostics); second, the bbjcpl compiler is designed to provide ground-truth semantic validation for completions that pass parsing -- a concept proven by the bbjcpltool v1 proof-of-concept, with production pipeline integration planned for Phase 2. Suggestions that fail either validation tier are silently filtered, ensuring that hallucinated code never reaches the developer.
